...
export class NotesListResolver {
  ...
  async addNote(
    ...
    const collection = await collectionsRepo.findOne(
      { id: collectionId, owner: req.session.userId },
      ["owner", "lists"]
    );
  ...
  async note(
    ...
    const notesList = await notesListRepo.findOne(
      { id: listId, collection: collection },
      ["collection"]
    );
  ...
  async deleteNotesList(
    ...
    const notesListToDelete = notesLists.filter(
      (list) => list.id === listId
    )[0];
  ...
  async moveList(
    ...
    const collection = await collectionRepo.findOne(
      { id: listLocation.collectionId, owner: req.session.userId },
      ["owner", "lists"]
    );
    ...
    const notesList = await notesListRepo.findOne({ id: listLocation.listId }, [
      "collection",
    ]);
    ...
    const newCollection = await collectionRepo.findOne(
      { id: newCollectionId, owner: req.session.userId },
      ["owner", "lists"]
    );
